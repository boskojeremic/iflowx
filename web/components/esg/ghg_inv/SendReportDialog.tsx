...
export default function SendReportDialog({
  ...
  async function handleSend() {
    try {
      ...
      const res = await fetch("/api/esg/ghg_inv/send-report-email", {
        method: "POST",
        headers: {
  ...
  return (
    <>
      <button
        onClick={handleOpen}
        className="rounded-md border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-300 hover:bg-emerald-500/15 transition"
      >
        Send as Attachment
      </button>
              ...
              <div className="max-h-[75vh] overflow-y-auto px-5 py-4">
                <div className="grid gap-4">
                  ...
                  <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3 text-sm text-white/70">
                    Attachment: {reportCode}_{reportDate}.pdf
                  </div>
                </div>
              </div>
