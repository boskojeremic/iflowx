...
setModuleId((prev) => {
  ...
  async function handleAdd() {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/core-admin/report-groups", {
        method: "POST",
  ...
  async function handleUpdate() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/core-admin/report-groups/${editingId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          code: upper(code).trim(),
  ...
  async function handleDelete(id: string) {
    setBusy(true);
    try {
      const res = await fetch(`/api/core-admin/report-groups/${id}`, {
        method: "DELETE",
      });
    ...
    <div className="space-y-4">
      <div className="w-full overflow-hidden">
        <div className="grid w-full grid-cols-12 gap-2 items-center">
          ...
          <input
            className="col-span-12 md:col-span-7 min-w-0 px-3 py-2 bg-black/40 border border-white/20 rounded-md"
            placeholder="NAME"
            value={name}
          ...
          {isEditMode && (
            <div className="col-span-12 md:col-span-2 flex justify-start">
              <AdminPrimaryButton
                onClick={cancelEdit}
                disabled={busy}
                variant="ghost"
