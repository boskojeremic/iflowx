...
import EditableMeasurementTable from "@/components/fop/EditableMeasurementTable";
import NewRevisionButton from "@/components/fop/NewRevisionButton";
import RevisionFilterSelect from "@/components/fop/RevisionFilterSelect";
...
const selectedDate = normalizeYmd(rawSelectedDate || yesterdayYmd());
  ...
  const historyReport = String(sp?.hreport ?? "").toUpperCase().trim();
    ...
    return (
      <div className="min-h-full space-y-4 overflow-y-auto p-4">
        <div className="text-sm text-white/50">
          <Link href="/ogi/fop" className="transition hover:text-white/80">
            Field Operations
          </Link>
...
const rejectedReportsCount = currentDayStatus?.status === "REJECTED" ? 1 : 0;
  ...
  const activePreviewTitle =
    ...
    activePreviewMeta.title;

  const activePreviewHtmlSrc = `/fop-preview/${activePreviewCode}?date=${selectedDate}${
  ...
  const historyStatusesRaw = tenant
    ? await db.reportDayStatus.findMany({
        where: {
          tenantId: tenant.id,
          reportDefinition: {
  ...
  const historySnapshots =
    tenant && historyReportIds.length > 0
      ? await db.measurementSnapshot.findMany({
          where: {
            tenantId: tenant.id,
            reportId: {
  ...
  for (const statusRow of historyStatuses) {
    ...
    const openHref = `/ogi/fop?tab=data-entry&rightTab=report&report=${statusRow.reportDefinition.code}&date=${ymd(
      statusRow.day
    )}&rev=${
        ...
        {mainTabs.map((t) => {
          ...
          return (
            <Link
              ...
              href={
                t.key === "history"
                  ? `/ogi/fop?${historyQueryString}`
                  : `/ogi/fop?tab=${t.key}&${baseQuery}`
              }
              className={[
              ...
              {rightTab === "charts" && (
                <div className="space-y-5">
                  <div className="grid gap-3 md:grid-cols-2">
                    <Link
                      href={`/ogi/fop?${checkPointBaseQuery}&cpMode=inputs&cpInputType=${checkPointInputType}`}
                      className={[
                    ...
                    <Link
                      href={`/ogi/fop?${checkPointBaseQuery}&cpMode=outputs`}
                      className={[
                        "rounded-2xl px-4 py-3 text-center text-sm font-semibold transition",
                        checkPointMode === "outputs"
                          ? "bg-white/10 text-white ring-1 ring-white/10"
                  ...
                  {checkPointMode === "inputs" && (
                    <div className="flex flex-wrap gap-2">
                      <Link
                        href={`/ogi/fop?${checkPointBaseQuery}&cpMode=inputs&cpInputType=manual`}
                        className={[
                      ...
                      <Link
                        href={`/ogi/fop?${checkPointBaseQuery}&cpMode=inputs&cpInputType=scada`}
                        className={[
                          "inline-flex items-center rounded-md px-3 py-1.5 text-xs font-semibold transition",
                          checkPointInputType === "scada"
                            ? "bg-blue-500/15 text-white ring-1 ring-blue-500/30"
                  ...
                  <div>
                    ...
                    <form method="GET" action="/ogi/fop" className="mt-2 space-y-2">
        ...
        <div className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          ...
          >
            ...
            <div className="grid gap-3 lg:grid-cols-[1.4fr_0.9fr_1fr_auto_auto]">
              ...
              <div className="flex items-end">
                <Link
                  href={`/ogi/fop?tab=history&report=${currentReportCode}&date=${selectedDate}${
                    selectedRevisionValue ? `&rev=${selectedRevisionValue}` : ""
                  }`}
    ...
    <div className="space-y-4">
      ...
      <div className="flex h-60 items-end gap-3">
        {points.map((p, idx) => {
          ...
          return (
            <div key={`${p.xLabel}_${idx}`} className="flex flex-1 flex-col items-center gap-2">
