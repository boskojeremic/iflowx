...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const previewUrl = `${baseUrl}/ghg_inv-preview/${reportCode}?date=${reportDate}`;

    await fs.mkdir(GHG_Inv_PDF_DIR, { recursive: true });

    const pngPath = path.join(
      GHG_Inv_PDF_DIR,
      `${reportCode}_${reportDate}.png`
    );
    const pdfPath = path.join(
      GHG_Inv_PDF_DIR,
      `${reportCode}_${reportDate}.pdf`
    );
    ...
    return NextResponse.json({
      ...
      pdfUrl: `/api/esg/ghg_inv/pdf/${reportCode}_${reportDate}.pdf`,
    });
  } catch (error) {
